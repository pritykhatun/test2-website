...
//    const {user} = useAuth();
   ...
   const onSubmit = data =>{
       console.log(data);
       axios.post('https://localhost:5000/food',data)
       .then(res=>{
...
<Container>
  <Row>
    <Col sm={12} md={12} lg={6}><img src={food.img} className='img-fluid mb-5'></img></Col>
    <Col sm={12} md={12} lg={6}>
    <form onSubmit={handleSubmit(onSubmit)} className="bg-dark text-center pt-4 pb-4 px-4 pe-4 mb-5">
     <input {...register("email", { required: true, maxLength: 120 })} placeholder="email" className='w-100 p-2'  />
<br></br>
<br></br>
     <input {...register("name", { required: true, maxLength: 120 })} placeholder="name" className='w-100 p-2' />
     <br></br>
<br></br>
